...
/**
 * @brief Run DPU Task for WideResNet50_2
 *
 * @param taskResnet50 - pointer to ResNet50 Task
 *
 * @return none
 */
void runWideResnet50_2(vart::Runner* runner) {
...
int main(int argc, char* argv[]) {
  // Check args
  if (argc != 2) {
    cout << "Usage of wide_resnet50_2 demo: ./wide_resnet50_2 [model_file]" << endl;
    return -1;
  }
  auto graph = xir::Graph::deserialize(argv[1]);
  auto subgraph = get_dpu_subgraph(graph.get());
  CHECK_EQ(subgraph.size(), 1u)
      << "wide_resnet50_2 should have one and only one dpu subgraph.";
